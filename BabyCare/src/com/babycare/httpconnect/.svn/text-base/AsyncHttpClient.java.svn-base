...
/**
 * 
 * 寮傛http璇锋眰 The AsyncHttpClient can be used to make asynchronous GET, POST, PUT
...
public class AsyncHttpClient {
	...
	private static final int DEFAULT_MAX_CONNECTIONS = 20;// 鏈�澶ц繛鎺ユ椂闂�
	private static final int DEFAULT_SOCKET_TIMEOUT = 20 * 1000;// timeout鏃堕棿
	...
	private final Map<Context, List<WeakReference<Future<?>>>> requestMap;// 寮傛鍥炶皟寮曠敤
	...
	public AsyncHttpClient() {
		...
		HttpConnectionParams.setSoTimeout(httpParams, socketTimeout);// 璇诲彇鏁版嵁
		HttpConnectionParams.setConnectionTimeout(httpParams, socketTimeout);// 杩炴帴瓒呮椂
		...
		httpClient.addResponseInterceptor(new HttpResponseInterceptor() {
			public void process(HttpResponse response, HttpContext context) {
				...
				final Header encoding = entity.getContentEncoding();// 鑾峰緱缂栫爜鏂瑰紡
				if (encoding != null) {
					for (HeaderElement element : encoding.getElements()) {
						// 濡傛灉鏈嶅姟鍣ㄦ槸zip鏂瑰紡鍙戦�佸垯杩涜鏁版嵁娴佺殑杞崲
		...
		threadPool = (ThreadPoolExecutor) Executors.newCachedThreadPool();// 鍒涘缓涓�涓彲鏍规嵁闇�瑕佸垱寤烘柊绾跨▼鐨勭嚎绋嬫睜

		requestMap = new WeakHashMap<Context, List<WeakReference<Future<?>>>>();// 浜嬩欢鍥炶皟鍑芥暟
	...
	public void get(Context context, String url, RequestParams params,
		...
		if (isNetworkConnected(context)) {
			...
			responseHandler.sendFailureMessage(new Exception(),
					GlobalApplication.NO_NETWORK);
		}*/
	...
	/**
	 *   澧炲姞涓�涓敤 map 瑁呰浇澶磋姹傜殑 鏂规硶  2014-7-17 
	 */
	public void getWithHead(Context context, String url,
			RequestParams params,Map<String, String> map ,AsyncHttpResponseHandler responseHandler) {
		if (isNetworkConnected(context)) {
			sendRequestWithHead(httpClient, httpContext, new HttpGet(
					getUrlWithQueryString(url, params)), map,null, responseHandler,
					context);
		} /*else {
			responseHandler.sendFailureMessage(new Exception(),
					GlobalApplication.NO_NETWORK);
...
/*		uriRequest.addHeader("Content-Type", GlobalApplication.request_head);*/
	 ...
	 * @param context
	 */
	protected void sendRequestWithHead(DefaultHttpClient client,
			HttpContext httpContext, HttpUriRequest uriRequest,Map<String, String> map,
			String contentType, AsyncHttpResponseHandler responseHandler,
			Context context) {
		
		for (String contentType1: map.keySet()) {
			uriRequest.addHeader(contentType1, map.get(contentType1));
		}
